$(document).ready(function() {
    ...
    $recipientType.addClass('select-select2').prepend('<option></option>').select2({
        placeholder: "Sélect. un type de compte ...",
    }).val('').trigger('change');
    $("#akyos_mailboxbundle_mail_recipient").addClass('select-select2').prepend('<option></option>').select2({
        placeholder: "Sélect. un destinataire ...",
    }).val('').trigger('change');
    // --> Mise à jour
    $recipientType.change(function() {
        ...
        $.ajax({
            ...
            success: function(html) {
                $('#s2id_akyos_mailboxbundle_mail_recipient').find('.select2-search-choice').remove();

                $('#akyos_mailboxbundle_mail_recipient').html(
                    $(html).find('#akyos_mailboxbundle_mail_recipient').html()
                );
                stopLoader(loader);
            }
        });
